/* autogenerated by Processing revision 1276 on 2021-09-19 */
...
public class MG_3_1 extends PApplet {
...
// is 600 by 600 pixels (see the call to size() in setup()).
 public void makePath(int pathNumber, boolean leftSide) {  
...
// scale them for the given origin, width, and height.
 public void fillPath(float originX, float originY, float wid, float hgt) {
...
// The array p is 2 elements long, and holds the x and y we find.
 public void pointOnPath(float fullPathT, float[] p) {
  // standard stuff for finding the right segment and t in that segment
  int numSegments = PApplet.parseInt((PathX.length-1)/3.0f);
  float tInSegments = fullPathT * numSegments;
  int firstIndex = 3 * PApplet.parseInt(tInSegments);
  float localT = tInSegments - PApplet.parseInt(tInSegments);
...
// array p with the x and y values of the curve at this point.
 public void pointOnSegment(int i, float t, float[] p) {  
...
// draw a bunch of points to create the "trail" it leaves behind.
 public void drawMovingPoint(int pathNumber, int clr) {
  float speed = pathNumber*1.4f/10000.0f;  // numbers tuned by eye 
 ...
 public void draw() {
  ...
  int bottomColor = color(253, 231, 76);
  int topColor = color(91, 192, 235);
  if (ShowStroke) stroke(0);
             else noStroke();
  for (int i=0; i<NumDrops; i+=2) {
     colorMode(HSB);
     int pointColor = lerpColor(bottomColor, 
...
smooth(); }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "MG_3_1" };
    if (passedArgs != null) {
